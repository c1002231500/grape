 ...
 //   $_url = $_SESSION['SmartyPaginate'][$_id]['url'];
    ...
    /*    } else {
        ...
        if($_item != $_curr_item) {
            ...
            $_ret .= $_link_prefix . '<a class="pagenavnum" href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . '>' . $_text . '</a>' . $_link_suffix;
        } else {
            $_ret .= $_link_prefix . "<span class='pagenavnumselected'>".$_text ."</span>". $_link_suffix;
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
