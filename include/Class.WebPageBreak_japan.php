...
class WebPageBreak1 {
	...
	// 分页信息样式
	private $mCssPageInfoNumFont = 'color:#7ca900';
	...
	private function GetPageIndex() {
		...
		for( $i=$first_number; $i<=$last_number; $i++ ) {
			if( $this->mCurPageNumber == $i ) {
				$page_index .= "<strong>"."<div style='width:30px; height:18px; text-align:center; line-height:18px;  background:#7ca900;border:1px solid #7ca900;margin-right:3px; float:left; margin-right:3px;cursor: pointer;color:#ffffff'>$i</div>"."</strong>";
			}
			else {
				$page_index .= "<a style='text-decoration: none; color:#7ca900;' href='".$this->mQueryString.$i."' ><div style='width:30px; height:18px; text-align:center; line-height:18px; margin-right:3px; float:left; border:1px solid #7ca900;margin-right:3px;cursor: pointer;'>".$i."</div></a> ";
			}
		}
//		var_dump($page_index);
		return $page_index;
	...
	private function GetNavSelectButton(){
        ...
        <div style='width:47px; height:20px; text-align:center; line-height:18px; margin-right:3px; float:left; _margin-right:3px;cursor: pointer;'>
        	<input id='number' name='number' style='width:47px; height:20px;background:url(/templates/Content/Public/images/go-bg.jpg) no-repeat;border:none;text-indent: 0.5em;line-height:18px;' type='text' >
        </div>
		<input style='width:47px;height:20px;float:left;border:none;' type='image' src='/templates/Content/Public/images/gopic.jpg' value='go' onclick='tiaozhuan()'>";
	} 
		

	// 得到页码索引条 首页按钮
	private function GetNavFirstButton() {
		return "<a style='text-decoration: none;color:#7ca900;' href='".$this->mQueryString."1'>".$this->mFirstButton."<div style='width:60px; height:18px; text-align:center; line-height:18px; border:1px solid #7ca900; margin-right:3px; float:left; margin-right:3px;cursor: pointer;'>最初</div></a> ";
	}
	...
	private function GetNavPreButton() {
		...
		return "<a style='text-decoration: none;color:#7ca900;' href='".$this->mQueryString.$pre_number."' class='page-prev'>".$this->mPreButton."<div style='width:60px; height:18px; text-align:center; line-height:18px; border:1px solid #7ca900; margin-right:3px; float:left; margin-right:3px;cursor: pointer;'>前へ</div></a> ";
	}
	...
	private function GetNavNextButton() {
		...
		return "<a style='text-decoration: none;color:#7ca900;' href='".$this->mQueryString.$next_number."' class='page-next' >".$this->mNextButton."<div style='width:60px; height:18px; text-align:center; line-height:18px; border:1px solid #7ca900; margin-right:3px; float:left; margin-right:3px;cursor: pointer;'>次へ</div></a> ";
	}

	// 得到页码索引条 末页按钮
	private function GetNavLastButton() {
		return "<a style='text-decoration: none;color:#7ca900;' href='".$this->mQueryString.$this->mTotalPagesNum."'>".$this->mLastButton."<div style='width:60px; height:18px; text-align:center; line-height:18px; border:1px solid #7ca900; margin-right:3px; float:left; margin-right:3px;cursor: pointer;'>最後
</div></a> ";
	}
	...
	private function SetPageInfo() {
		...
		$this->mPageInfo .= "全部で</font> <font style='".$this->mCssPageInfoNumFont."'>".$this->mTotalRowsNum."</font> の項目がありました。 | ";
//		$this->mPageInfo .= "<font style='".$this->mCssPageInfoNumFont."'>".$this->mPageRowsNum."</font> 条/页 | ";
		$this->mPageInfo .= "Aｌｌ <font style='".$this->mCssPageInfoNumFont."'>".$this->mTotalPagesNum."</font> Pages ";
